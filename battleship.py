...
class Field:
    ...
    def __eq__(self, otherField):
        if not isinstance(otherField, type(self)):
            return False

        return ((self._row == otherField._row) and (
            self._column == otherField._column))
...
class Board:
    ...
    def _getAvailableStartFields(self, shipSize):
        start = self._getFreeFields(
            shipSize, Direction.HORIZONTAL, lambda i, j: (
                i, j), (self._rowNumber, self._columnNumber))
        start.extend(
            self._getFreeFields(
                shipSize, Direction.VERTICAL, lambda i, j: (
                    j, i), (self._columnNumber, self._rowNumber)))
        return start
...
class HitBoard:
    ...
    def __init__(self, rowsNbr, columnsNbr):
        ...
        self._fields = {(r, c): HitField(r, c) for r in range(
            self._rowsNumber) for c in range(self._columnsNumber)}
    ...
    def _fieldsForHorizontalShip(self, shipSize):
        return self._shipFields(shipSize,
                                lambda i,
                                j: self._fields[i,
                                                j],
                                self._rowsNumber,
...
class Artillery:
    ...
    def processResults(self, shootingResults):
        ...
        self._fieldsShootShip.sort(
            key=lambda field: field._row + field._column)
...
class ChooseFieldAfterFirstHit:
    ...
    def chooseField(self):
        nearbyFields = [
            self._grid.fieldsSameDirection(
                self._hitField, FieldDirection.LEFT), self._grid.fieldsSameDirection(
                self._hitField, FieldDirection.UP), self._grid.fieldsSameDirection(
                self._hitField, FieldDirection.RIGHT), self._grid.fieldsSameDirection(
                    self._hitField, FieldDirection.DOWN)]
...
class ChooseFieldSystematicShooting:
    ...
    def chooseField(self):
        ...
        if firstField._row == lastField._row:
            ...
            nearby = [
                self._grid.fieldsSameDirection(
                    firstField, FieldDirection.UP), self._grid.fieldsSameDirection(
                    firstField, FieldDirection.DOWN)]
